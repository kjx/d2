...
lemma COKAMFO(oo : Owner, context : set<Object>)
  ...
  ensures  CallOK(flatten(oo), context)
{
  reveal COK();
  reveal CallOK();

forall x <- oo ensures ( CallOK(flatten({x}), context) ) //by
  {
    assert COK(x, context);
//    COKownerAMFOs(x, context);
    assert CallOK(x.AMFO, context);
    assert CallOK(flatten({x}), context);
  }
 assert CallOK(flatten(oo), context);
}
...
lemma COKExtraContext(a : Object, context : set<Object>, extra : set<Object>)
  //requires a in context
  requires COK(a,context)
  ensures COK(a,context + extra)
    { COKWiderContext(a, context, extra); }
  ...
  //modifies {}
{
  ...
  assert forall x <- context :: x.Context(context);
...
lemma COKfromHeapContextReady(o : Object, m : Klon)
...
{
  reveal COK(), CallOK();
  assert forall x <- m.oHeap :: (x.Ready() && x.Valid() && x.Context(m.oHeap)) by { assert  m.HeapContextReady(); }
  assert forall a <- m.oHeap :: COK(a,m.oHeap);
 }
...
lemma CallOKNarrowContext(aa : set<Object>, less : set<Object> := aa,  more : set<Object>)
  requires less <= more
  requires CallOK(aa, more)
  requires forall a <- aa :: a.Context(less)
   ensures CallOK(aa, less)
  {
    reveal CallOK();
    reveal COK();
    assert forall a <- aa :: COK(a,less);
  }
...
lemma OneMoreHeap(a : Object, m : Klon, m' : Klon)
