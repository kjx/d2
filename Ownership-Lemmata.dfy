...
lemma SubAMFOsNonU(partO : OWNR, wholeO : OWNR)
...
lemma FlattenExtraReady(o : Object)
     requires o.Ready()
...
lemma InsideObjectsInsideOwners1(f : Object, t : Object)
  requires f.Ready()
  requires t.Ready()
...
lemma abcd(a : Object, b : Object, c : Object, d : Object, m : Klon)
  requires a.Ready()
  requires b.Ready()
  requires c.Ready()
  requires d.Ready()
...
lemma ac(a : Object, c : Object, m : Klon)
  requires klonReady(m)
  requires klonCalid(m)
...
function recFlatOwn0(o : Object) : (rv : Owner)
  decreases o.AMFO
   requires o.Ready()
...
function recFlatOwn(o : Object) : (rv : Owner)
  decreases o.AMFO
   requires o.Ready()
    ensures rv == o.AMFO
...
//
// function {:timeLimit 15} flatten(os : Owner) : (fs : Owner)
//      reads {}
//    ensures os <= fs
...
function {:timeLimit 15} flattenRFO(os : Owner) : (fs : Owner)
...
lemma {:timeLimit 30} FlatRFOIsFlatten(os : Owner)
...
lemma {:timeLimit 10} SetRFOIsSetAMFO(os : Owner)
...
//
//
// function {:timeLimit 15} recFlatten(os : Owner) : (fs : Owner)
...
//
//
// function rfo(o : Object) : (rv : Owner)
//   decreases o.AMFO
//    requires o.Ready()
//     {  {o} + (set xo <- o.owner, co <- rfo(xo) :: co)  }
//
//
// lemma {:timeLimit 30} FUCKED_RecFlattenIsFlatten(os : Owner)
//   decreases allAMFOs(os)
//    requires AllReady(os)
...
//
//
// lemma {:timeLimit 30} FUCKED_OneObjectReCFLATTEN(o : Object)
//   decreases o.AMFO
//    requires o.Ready()
...
function {:timeLimit 30} mflat(oo : Owner) : Owner  decreases allAMFOs(oo), 2 requires forall o <- oo :: o.Ready()  {set o : Object <- oo, ooo <- mamfo(o) :: ooo}
...
lemma FLATTENtoAMFO(o : Object)
 ...
 lemma {:timeLimit 15} FRFOFLAT(os : Owner)
  decreases allAMFOs(os)
   requires AllReady(os)
   requires forall o <- os :: o.Ready()
    ensures flattenRFO(os) == flatten(os)
...
//
//
//  lemma {:timeLimit 15} RecFlattenIsFlatten(os : Owner)
//   decreases allAMFOs(os)
//    requires AllReady(os)
//     ensures recFlatten(os) == flatten(os)
//     {
