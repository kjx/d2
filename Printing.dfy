...
method {:isolate_assertions} printklon(m :  Klon)
  ...
  ensures unchanged(m.oHeap,m.hns())
...
lemma DAFNY_IS_SATAN(s: set<string>)
  requires s > {}
   ensures forall m <- s, n <- s |
              (&& (forall x <- s :: strLEQ(m, x))
               && (forall x <- s :: strLEQ(n, x))) :: m == n
{
  ...
  OneRessurection(s,min);
  assert forall x <- s | IAMTheRessurection(s,x) :: x == min;
  assert forall m <- s, n <- s |
              (&& IAMTheRessurection(s,m)
               && IAMTheRessurection(s,n)) :: min == m == n;
  assert forall m <- s, n <- s |
              (&& (forall x <- s :: strLEQ(m, x))
               && (forall x <- s :: strLEQ(n, x))) :: m == n;
}
...
lemma  {:isolate_assertions} SetStrHasMinimum(s : set<string>)
  requires |s| > 0
   ...
   ensures exists m : string <- s :: forall x <- s ::  strLEQ(m, x)
{
   forall m <- s ensures strLEQ(m, m)  { StrEQISstrLEQ(m,m); }

   var min :| min in s;
   var done := {min};
   var rest := s - done;
   assert forall x <- done :: strLEQ(min, x);

   while (rest > {})
     decreases rest
     invariant rest == (s - done)
     invariant forall x <- done :: strLEQ(min, x)
     invariant min in s
     {
      var next :| next in rest;
      assert forall x <- done :: strLEQ(min, x);
      if not(strLEQ(min, next)) {
          StrNEQLEQ(min, next);
          assert strLEQ(next, min);
          forall x <- done ensures strLEQ( next, x)
            { StrLEQIsTrans(next, min, x); }
          min := next;
          assert strLEQ(min, next);
          assert forall x <- done :: strLEQ(min, x);
          assert forall x <- done+{next} :: strLEQ(min, x);
      }
      done := done + {next};
      rest := rest - {next};
      assert rest == s - done;
...
lemma  {:isolate_assertions} SetStrMin(s : set<string>) returns (min : string)
  requires |s| > 0
   ...
   ensures forall m <- s ::  strLEQ(m, m)
   ensures exists m : string <- s :: forall x <- s ::  strLEQ(m, x)
   ensures forall x <- s :: strLEQ(min, x)
   ensures min in s
{
   forall m <- s ensures strLEQ(m, m)  { StrEQISstrLEQ(m,m); }
   ...
   while (rest > {})
     ...
     SetMinus0(s,done);
     assert s <= done;
     assert forall x <- s :: strLEQ(min, x);
}
...
lemma StrNEQLEQ(l : string, r : string)
  ensures  strLEQ(l,r) || strLEQ(r,l)
  {}
