...
lemma FewerObjectsFewerValidEdges(lesso : set<Object>, moreo : set<Object>)
  requires lesso <= moreo
  ensures edges(lesso) <= edges(moreo)

  requires (var less := edges(lesso);
            var more := edges(moreo);
            var lesp := partitionedIncomingEdges(less);
            var morp := partitionedIncomingEdges(more);


            && edgesAreConsistentWithDafnyHeap(less)  //no-op!
            && edgesAreConsistentWithDafnyHeap(more)  //no-op!

            && OnlyOneOwnedOrLoanedIncoming(morp)
            && OneOwnerIncoming(morp)
            && BorrowedNotOwnedIncoming(morp)
            && BorrowsLoansConsistentPermissionIncoming(morp)
            && OnlyOneWriterIncoming(morp)

            && IncomingReferencesConstraintsOK(more)
            )

  ensures (var less := edges(lesso);
            var more := edges(moreo);
            var lesp := partitionedIncomingEdges(less);
            var morp := partitionedIncomingEdges(more);

            && OnlyOneOwnedOrLoanedIncoming(lesp)
            && OneOwnerIncoming(lesp)
            && BorrowedNotOwnedIncoming(lesp)
            && BorrowsLoansConsistentPermissionIncoming(lesp)
            && OnlyOneWriterIncoming(lesp)

            && partitionedLessEQ(lesp, morp)
            && IncomingReferencesConstraintsOK(less)
            )
{
  var less := edges(lesso);
  var more := edges(moreo);
  var lesp := partitionedIncomingEdges(less);
  var morp := partitionedIncomingEdges(more);
  FewerIsLess(lesso,moreo);
  partitionedIncomingEdgesMonotonic(less, more, lesp, morp);

  assert partitionedLessEQ(lesp, morp);
  OnlyOnePredIncomingMonotonic(OwnedOrLoanedEdge, lesp, morp);
  OnlyOnePredIncomingMonotonic(WriterEdge,        lesp, morp);
}
  //
  // requires lesp == partitionedIncomingEdges(edges(lesso))
  // requires morp == partitionedIncomingEdges(edges(moreo))
