...
function mapKV<K(==),V(==)>(m' : map<K,V>, k : K, v : V) : (m : map<K,V>)
  //m==m'[k:=v]
    requires k !in m'.Keys
     ...
     ensures m[k] == v
     ensures forall z <- m'.Keys :: m[z] == m'[z]
     ensures forall z <- m.Keys ::
      if (z == k)
        then (m[z] == v)
        else (m[z] == m'[z])
  {
...
lemma SetSubtractDisjoint<T>(o : set<T>, p : set<T>, r : set<T>)
  requires r == o - p
  ensures  r !! p
{}
...
lemma MapGLEQSameInnit<K,V>(left : map<K,V>, l : K, right : map<K,V>, r : K)
  requires mapGEQ(left,right)
  requires l in left.Keys
  requires r in right.Keys
  requires l == r
   ensures left[l] == right[r]
  {}


predicate SetElementsEq<T>(aa : set<T>, bb : set<T>) : (r : bool)
  ensures  r == (aa == bb)
  {
  && (forall a <- aa :: a in bb)
  && (forall b <- bb :: b in aa)
...
predicate MapElementsEq<K,V(==)>(aa : map<K,V>, bb : map<K,V>) : (r : bool)
  ensures  r == (aa == bb)
  {
  && (forall a <- aa.Keys :: (a in bb.Keys) && (aa[a] == bb[a]))
  && (forall b <- bb.Keys :: (b in aa.Keys) && (aa[b] == bb[b]))
  }
...
lemma SameIsEqual<T>(aa : set<T>, bb : set<T>)
//see SetElementsEq()
  requires forall a <- aa :: a in bb
  requires forall b <- bb :: b in aa
   ensures  aa == bb
  {}
...
//james' attempt at the full version with all the working
...
lemma IfAllValuesAreTheirOwnKeysThenAllKeysAreAllValues<E>(m : map<E,E>)
  requires forall k <- m.Keys :: m[k] == k
...
//inVertible map
//umap or bimap or isomap or isomorph would make more sense.
...
//this one seems to work better, not sure why…
predicate AllMapEntriesAreUnique<K,V(==)>(m : map<K,V>)
  //all map entries are unique
{
  forall i <- m.Keys, j <- m.Keys :: (m[i] == m[j]) ==> (i == j)
}

predicate AllMapEntriesAreUnique2<K,V(==)>(m : map<K,V>)
{
    reveal UniqueMapEntry();
...
lemma AMEAU1<K,V>(m : map<K,V>)
  requires AllMapEntriesAreUnique(m)
   ensures  AllMapEntriesAreUnique2(m)
    { reveal UniqueMapEntry(); }

lemma AMEAU2<K,V>(m : map<K,V>)
  requires AllMapEntriesAreUnique2(m)
   ensures  AllMapEntriesAreUnique(m)
    { reveal UniqueMapEntry(); }
     ...
     ensures  forall k <- n.Keys :: m[ n[k] ] == k
{
      ...
      var n := map k <- m.Keys :: m[k] := k;

      assert n.Keys   == (set k <- m.Keys :: m[k])             == m.Values;
    //  assert n.Values == (map k <- m.Keys :: m[k] := k).Values == m.Keys;


      assert forall k <- m.Keys :: m[k] in n.Keys;
      assert forall k <- n.Keys :: k in m.Values;
...
lemma InversionHate<K,V>(m : vmap<K,V>, n : vmap<V,K>)
 //needs a better name this just goes in the opposite directin to inversionLove
  requires forall k <- m.Keys :: m[k] in n.Keys
  requires forall k <- m.Keys :: n[m[k]] == k
  requires forall k <- n.Keys :: n[k] in m.Keys
  requires forall k <- n.Keys :: m[n[k]] == k
...
lemma InversionBothWays<K,V>(m : vmap<K,V>, n : vmap<V,K>)
  requires n == invert(m)
   ensures  m == invert(n)
{}

lemma InversionDoubler<K,V>(m : vmap<K,V>, n : vmap<V,K>)
  requires n == invert(m)
   ensures  m == invert(invert(m))
{}


lemma InversionKeysValues<K,V>(m : vmap<K,V>, n : vmap<V,K>)
  requires n == invert(m)
  requires m == invert(n)
   ensures  n.Keys   == m.Values
   ensures  n.Values == m.Keys
...
lemma InversionMappage<K,V>(ks : set<K>, vs : set<V>, m : vmap<K,V>, n : vmap<V,K>)
  requires n == invert(m)
   ensures m == invert(n)
   ensures n.Keys   == m.Values
   ensures n.Values == m.Keys
  ...
  requires ks <= m.Keys
  requires vs <= n.Keys
  requires mapThruVMap(ks, m) == vs

   ensures mapBackVMap(vs, m) == ks
   ensures mapThruVMap(vs, invert(m)) == ks
   ensures mapBackVMap(ks, invert(m)) == vs
{}
...
function AnEmptyMapIsAnUniqueMap<K,V>(m : map<K,V>) : (u : vmap<K,V>)
    requires m == map[]
        { m }
...
function mapThruVMap<K,V>(ks : set<K>, m : vmap<K,V>) : (vs : set<V>)
    requires ks <= m.Keys
     ensures  vs <= m.Values
    reads {}
  { set k <- ks :: m[k] }
...
lemma LookupAllMapGEQ<K,V>(m : map<K,V>, m' : map<K,V>)
  requires mapGEQ(m,m')
   ensures  m.Keys >= m'.Keys
   ensures  forall k <- m'.Keys :: k in m.Keys
   ensures  forall k <- m'.Keys :: m[k] == m'[k]
{}

lemma MapThruVMapElements<K,V>(ks : set<K>, m : vmap<K,V>, vs : set<V>)
  requires ks <= m.Keys
  requires mapThruVMap(ks, m) == vs
   ensures forall ko <- ks :: m[ko] in vs
   ensures forall vo <- vs :: invert(m)[vo] in ks
{}

lemma MapWithoutKey<K,V>(m : vmap<K,V>, ks : set<K>, vs : set<V>, k : K, v : V)
   //yes, named after the canadian 1980s pop band
   //argument order?
  requires ks <= m.Keys
  requires mapThruVMap(ks,m) == vs
  requires k in ks
  requires m[k] == v
   ensures mapThruVMap(ks-{k},m) == vs-{v}
  {}
