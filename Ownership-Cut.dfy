...
lemma {:timeLimit 15} CutIsCut(cut : Object, tree : set<Object>, trunk : set<Object>, branch : set<Object>)
  requires cut in tree
...
// {:timeLimit 60}
lemma {:vcs_split_on_every_assert} {:timeLimit 15} CutIsCut2(cut : Object, tree : set<Object>, trunk : set<Object>, branch : set<Object>)
  requires forall o <- tree :: o.Ready() && o.Valid()
  requires cut in tree
  requires branch == set o <- tree |     inside(o,cut)  :: o
  requires trunk ==  set o <- tree | not(inside(o,cut)) :: o
  ...
  requires StandaloneObjectsAreValid(tree)
   ...
   ensures cut in branch
//   ensures forall f <- trunk, t <- f.outgoing() :: (t in trunk) || (t == cut)
//
...
//     ensures true
   {
      assert refOK(f,t);  //comes in because all objects are Valid()

      if (f !in trunk) { assert f in branch; }
      else if ((t in trunk) || (t == cut)) { skip(); }
      else {
        assert f in trunk;
        assert t in branch;
        assert t != cut;

        assert f.Ready() && cut.Ready() && t.Ready();
        assert strictlyInside(t,cut);
        assert outside(f,cut);
        assert outside(t,f) by
           {
            assert f in trunk; assert outside(f,cut);
            assert t in branch; assert strictlyInside(t,cut);
           }
        NoIncomingPointers(f,cut,t);
...
lemma {:isolate_assertions} SeperationOfPowers(cut : Object, tree : set<Object>, trunk : set<Object>, branch : set<Object>, f : Object, t : Object)
  ...
  requires forall b <- branch, t <- trunk :: inside(b, t) ==> inside(cut, t)  //hmm not enough...

  requires forall b <- branch :: not(exists x <- trunk :: b in x.outgoing())

  requires f in trunk
  requires t in branch
  requires t != cut

  ensures strictlyInside(t,cut)
  ensures outside(f,cut)
  ensures outside(f,t)
 //x` ensures outside(t,f)   ///****************************************************************
{
  assert t.AMFO >= cut.AMFO;
  assert not(f.AMFO >= cut.AMFO);
...
//           assert inside(t, cut);
//
//           if (t != cut)   // (ownerInsideOwner(t.owner, {cut}) && t != cut)
//           {
//             assert refOK(f,t);
//             assert strictlyInside(t,cut);
//   assert f in t.owner;  DreddOwner(f,t);
//
//             //assert {:contradiction} ownerInsideOwner({f},t.owner);
//             assert inside(t,f);   assert directlyInside(t,f);
//             // assert inside
//            // transitiveInsideOwners(t.owner, {f}, {cut});
//             assert {:contradiction} not(inside(f,cut));
// //            assert false;
//           } else {
//             assert f in trunk;
//             assert inside(t, cut);
...
lemma {:vcs_split_on_every_assert} CuttyOne(cut : Object, tree : set<Object>, trunk : set<Object>, branch : set<Object>)
